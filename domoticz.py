...
class Domoticz:
    ...
    def updateElectricityMeter(self, idx, power, energy):
        self.dzRequest(
            "http://" + Configuration.dzServer +
            "/json.htm?type=command&param=udevice&idx=%d" % idx +
            ("&nvalue=0&svalue=%d" % power) + (";%d" % energy) +
...
if __name__ == '__main__':
    dz = Domoticz()
    dz.updateElectricityMeter(Configuration.dzEnergyMeterDeviceId, 0, 567000)
